...
def read_inventory():
    ...
    for raw_line in INVENTORY_PATH.read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("- code: "):
            ...
            current = {"code": line[len("- code: "):].strip(), "status": "unused"}
...
def seed_codes(count):
    ...
    entries = [{"code": code, "status": "unused"} for code in codes]
...
def replenish_codes(count):
    ...
    entries.extend({"code": code, "status": "unused"} for code in codes)
    write_inventory(entries)
